...
// Fetch seat data ensuring correct showtime and date filtering
$sql = "SELECT s.id, s.seat_number, s.row_label, s.status, s.category
        FROM seats s
...
<head>
    ...
    <style>
        .screen { width: 80%; margin: 20px auto; text-align: center; font-weight: bold; padding: 10px; background: #444; color: white; border-radius: 5px; }
        .seat-container { display: flex; flex-direction: column; align-items: center; gap: 15px; }
        ...
        .selected { background-color: #ffc107 !important; color: black; } /* Yellow */
        .unavailable { background-color: #dc3545 !important; color: white; pointer-events: none; } /* Red */
    ...
    <div class="container">
        ...
        <div class="seat-container">
            <?php foreach ($seats as $category => $rows) { ?>
                <h5 class="text-center">Category: <?= ucfirst($category); ?></h5>
                <?php foreach ($rows as $row_label => $row_seats) { ?>
                    <div class="row">

...
$sql = "SELECT theaters.id AS theater_id, theaters.name AS theater_name, theaters.location, 
               screens.screen_name, showtimes.id AS showtime_id, showtimes.time, 
    ...
    while ($row = $result->fetch_assoc()) {
       ...
       if ($row['total_seats'] > 0) {
        ...
        if ($availability_percentage >= 100) {
            ...
            $availability_level = "high"; // Green
        }
    } else {
        $availability_level = "full"; // No seats at all
    }
...
<head>
    ...
    <style>
        ...
        .showtime-btn.full, .showtime-btn.disabled { background: #dc3545; color: white; /*pointer-events: none; opacity: 0.5; */} 
    </style>
</head>
<body>
    <div class="container">
            ...
            <?php foreach ($theaters as $theater => $data) { ?>
                <div class="showtime-container">
                    ...
                    <?php foreach ($data['screens'] as $screen => $shows) { ?>
                        ...
                        <?php 
                        $current_datetime = new DateTime(); // Get current date & time
                        foreach ($shows as $show) { 
                            ...
                            $is_past = ($show_datetime < $current_datetime) ? "disabled" : "";
                            ?>
                        <?php if ($availability_level == "full") { ?>
    <span class="btn showtime-btn full disabled" style="background: #dc3545; color: white; pointer-events: none; opacity: 0.8;"><?php echo htmlspecialchars($show_time); ?> (Houseful)</span>
<?php } elseif ($is_past) { ?>
    <span class="btn showtime-btn disabled" style="background: #6c757d; color: white; pointer-events: none; opacity: 0.5;">
    <?php echo htmlspecialchars($show_time); ?> (Time Over)
</span>
<?php } else { ?>
    <a href="seat_selection.php?movie_id=<?php echo $movie_id; ?>&movie=<?php echo urlencode($movie_title); ?>&theater_id=<?php echo urlencode($data['id']); ?>&theater=<?php echo urlencode($theater); ?>&screen=<?php echo urlencode($screen); ?>&time=<?php echo urlencode($show_time); ?>&date=<?php echo urlencode($selected_date); ?>" 
       class="btn showtime-btn <?php echo $availability_level; ?>"

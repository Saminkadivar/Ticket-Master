...
// Fetch movie, theater, screen, and seat details
$sql = "SELECT m.title AS movie_title, t.name AS theater_name, sc.screen_name, st.time, st.date, s.seat_number, s.id AS seat_id 
        FROM seats s
...
$amount = count($selected_seats) * 200;
...
<head>
    ...
    <style>
        ...
        .container { max-width: 600px; margin: 0 auto; padding: 20px; background-color: white; border-radius: 8px; text-align: center; }
        .btn { background-color: #3aafa9; color: white; padding: 10px 20px; border-radius: 5px; cursor: pointer; }
        .btn:hover { background-color: #2d8e8b; }
    </style>
</head>
<body>
    <div class="container">
    ...
    <script>
   function payNow() {
       var amount = <?= $amount * 100; ?>; // Convert to paisa
       ...
       var options = {
           ...
           "handler": function (response) {
               ...
               // Debugging: Check if paymentId exists
